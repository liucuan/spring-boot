package com.tone.github.cache.support;
...
public class KeyLock<K> {
    ...
    // 保存每个线程锁定的KEY及其锁定计数
    private final ThreadLocal<Map<K, LockInfo>> local = ThreadLocal
        .withInitial(() -> new HashMap<>());

    /**
     * 锁定key，其他等待此key的线程将进入等待，直到调用{@link #unlock(K)} 使用hashcode和equals来判断key是否相同，因此key必须实现{@link
     * #hashCode()}和 {@link #equals(Object)}方法
     */
